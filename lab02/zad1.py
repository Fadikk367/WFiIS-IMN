...
def picard_iteration(u_prev: float, u_picard_prev: float) -> float:
  return u_prev + dt/2.0 * ((alpha*u_prev - Betha*u_prev**2) + (alpha*u_picard_prev - Betha*u_picard_prev**2))


def iteratively_enhance_solution(u_prev: float) -> float:
  u_picard_curr = u_prev
  u_picard_prev = u_prev

  iter_count = 0
  while True:
    u_picard_prev = u_picard_curr
    u_picard_curr = picard_iteration(u_prev, u_picard_prev)
    iter_count += 1
    if iter_count > MAX_ITER_COUNT or is_within_tolerance(u_picard_curr, u_picard_prev):
      break
  
  return u_picard_curr
...
if __name__ == "__main__":
  ...
  for i in np.arange(1, n):
    u_picard_curr = iteratively_enhance_solution(u[i-1])
    u[i] = u[i-1] + dt/2.0 * (fun((i-1)*dt, u[i-1]) + fun(i*dt, u_picard_curr))
    z[i] = N - u[i]
